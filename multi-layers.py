...
class Calculator:
    def __init__(self):
        ...
        Label(frame, text="Number of Layers",bg='#F0FFFF').grid(row=11, column=1, columnspan=3, sticky=W)
...
#Input Values
        ...
        Entry(frame, justify=RIGHT, textvariable=self.NL).grid(row=11, column=3, sticky=W)
...
#Units
        ...
        Message(frame, text="Unitless").grid(row=11, column=5)
...
#calculate the volume
            if NL %2 ==0:
                ini_n_1 = NL-1   
                Number_Layer = int(NL/2)
                for o in range (Number_Layer):
                    V.append(ini_n*2*x*y/(NL*NL))
                    L.append(2*L1/NL)
                    U.append((H+y*ini_n/NL-WL)*Yw*L[o]*g)
                    ini_n +=2
                for m in range (Number_Layer):
                    No = int(m+ NL/2)
                    V.append(ini_n_1*2*h2*LL/(NL*NL))  
                    L.append(2*L2/NL)
                    U.append((h-h*ini_2/NL-WL)*Yw*L[No]*g)
                    ini_n_1 -=2
                    ini_2 +=2
            else: 
                ini_n_1 = NL  
                Number_Layer1 = int((NL-1)/2)
                Number_Layer2 = int((NL+1)/2)
                for o in range (Number_Layer1):
                    V.append(ini_n*2*x*y/((NL-1)*(NL-1)))
                    L.append(2*L1/(NL-1))
                    U.append((H-y*ini_n/(NL-1)-WL)*Yw*L[o]*g)
                    ini_n +=2
                for m in range (Number_Layer2):
                    No = int(m+ NL/2)
                    V.append(ini_n_1*2*h2*LL/((NL+1)*(NL+1)))
                    L.append(2*L2/(NL+1))
                    U.append((h-h*ini_2/(NL+1)-WL)*Yw*L[No]*g)
                    #print(No)
                    ini_n_1 -=2
                    ini_2 +=2
            L_tol = sum(L)
...
#Find the lowest Fs with Beta
        ...
        elif Fsf[2]> Fsf[0] and Fsf[1]>Fsf[0]:
            y_low = Fsf[0]
            Beta_results = Beta[0]/math.pi*180
        elif Fsf[1]>Fsf[2] and Fsf[0]>Fsf[2]:
            y_low = Fsf[2]
